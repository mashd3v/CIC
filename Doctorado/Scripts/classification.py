...
class MachineLearning:
    ...
    def get_best_cross_validation(self):
        ...
        for i, subset in enumerate(train_subsets):
            if subset.shape[0] != 0:
                ...
                for j, model in enumerate(self.models):
                    ...
                    precision_result = round(
                        final_model.cv_results_["mean_test_Precision"][0], 4
                    )
        ...
        for i, best_model_label in enumerate(best_models_per_label):
            joblib.dump(
                best_model_label.best_estimator_,
    ...
    def one_vs_rest_classifier(self):
        ...
        for i, model in enumerate(self.models):
            ...
            X_features = self.vectorizer.fit_transform(
                data[text_column].apply(lambda x: np.str_(x))
            )
