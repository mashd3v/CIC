...
class BestHyperparams:
    def __init__(
        self, ngram_ranges, min_dfs, data_train, data_test, text_column, class_column
    ):
        ...
        for ngram_range in self.ngram_ranges:
            for min_df in self.min_dfs:
                ...
                X_train_tfidf = vectorizer.fit_transform(
                    self.data_train[self.text_column]
                )
                X_test_tfidf = vectorizer.fit_transform(
                    self.data_test[self.text_column]
                )
                        ...
                        [
                            ...
                            {"type": "nb", "alpha": hp.uniform("alpha_nb", 0, 1)},
                            {
                                ...
                                "n_estimators": hp.choice(
                                    "n_estimators_rf", range(50, 200, 10)
                                ),
                            },
                        ],
                    )
                ...
                for trial in trials.trials:
                    if (
                        best_params is None
                        or trial["result"]["loss"] < best_params["loss"]
                    ):
            ...
            print(
                f"Ngram Range: {ngram_range} - Min DF: {min_df} - Best Model: {clf_type} - Accuracy: {acc}"
            )
